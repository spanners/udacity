...
def hasenough(transaction, buyers):
    return buyers[name(transaction)] >= amount(transaction)

def forsale(item,sellings):
    return item in sellings

def validexchange(transaction,sellings):
    return amount(transaction) == sellings[item(transaction)]

def make_sale(itm, buyer, accounts, ast):
    for cmd in ast:
        if issell(cmd) and item(cmd) == itm:
            accounts[name(cmd)] += amount(cmd)
            accounts[buyer] -= amount(cmd)
            ast.remove(cmd)
            for cmd in ast:
                if isbuy(cmd) and item(cmd) == itm:
                    ast.remove(cmd)
    return ast, accounts

def get_accounts(ast):
    return dict([(name(c), amount(c)) for c in ast if ishas(c)])

def eval_helper(ast, accounts):
    if len([c for c in ast if isbuy(c)]) == 0:
        return accounts
    else:
        sellings = dict([(item(c), amount(c)) for c in ast if issell(c)])
        for cmd in [c for c in ast if not ishas(c)]:
            if isbuy(cmd) and hasenough(cmd,accounts):
                if forsale(item(cmd),sellings) and validexchange(cmd,sellings):
                    ast, accounts = make_sale(item(cmd), name(cmd), accounts, ast)
                    return eval_helper(ast, accounts)
    return accounts

def evaluate(ast):
    return eval_helper(ast, get_accounts(ast))
