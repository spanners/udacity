...
def nfsmaccepts(current, edges, accepting, visited):
    ...
    unvisited = [edge for edge in edges if edge[0] not in visited]
    letter = [letter for (state, letter) in edges if state == current][0]

    if current in accepting:
        return ""
    else:
        for uedge in unvisited:
            destinations = edges[uedge]
            for state in destinations:
                if state not in visited:
                    return uedge[1] + nfsmaccepts(state, edges, accepting, visited)
                else:
                    return letter
 #       return [nfsmaccepts(state, unvisited_edges, accepting, visited) for state in
 #               unvisited_states]
...
print "poop", nfsmaccepts(1, edges2, accepting2, [])
