...
def p_exp_letter_sequence(p):
    'exp : LETTER sequence'
    p[0] = ("characters", [p[1]] + p[2])
...
def p_exp_zeromore(p):
    'exp : exp ZEROMORE'
    p[0] = ("zeromore", p[1])

def p_error(p):
    tok = yacc.token
    print "RegExp Syntax Error: Near Token " + str(tok)
    exit(1)

def interpret(ast):
    """the regexp ((ab)|(cd))* gives the below ast:
    ('zeromore', ('group', ('or', ('group', ('characters', ['a', 'b'])),
    ('group', ('characters', ['c', 'd'])))))"""
    edges = {}
    for elt in ast:
        etype = elt[0]
        if etype == 'characters':
