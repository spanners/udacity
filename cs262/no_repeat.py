...
def can_optimise(rhs, ast):
    """
    Determine if none of the the variables involved
    in 'right_hand_Side' have changed.
    """
    vars_involved = [var(x) for x in rhs if x[0] == "identifier"]
    c = [x[1] for x in ast]
    changes = any([True for x in c if x in vars_involved])
    if not changes:
        return True
    return False
...
def is_matchingRHS(i, ast):
    return RHS(ast[i]) in [RHS(x) for x in ast[i+1:]]

def optimize(ast):
    for i in range(len(ast)-1):
        if is_matchingRHS(i,ast):
            match = [x for x in rest(i,ast) if RHS(x) == RHS(ast[i])]
            pos = ast.index(match[0])
            if can_optimise(RHS(match[0]), ast[i+1:pos]):
                ast[pos] = ('assign', var(match[0]), ("identifier", var(ast[i])))
                if i + 1 == len(ast):
                    return ast
                else:
                    return optimize(ast)
